...
import { Pagination, Button, Tag, Select, Switch } from "antd"
...
import { useAPI, DataType } from "hooks/useAPI"
...
const NewTabIcon = () => (
	<svg version="1.0" xmlns="http://www.w3.org/2000/svg" width="16px" height="16px" viewBox="0 0 9810 9820" preserveAspectRatio="xMidYMid meet">
...
const FileViewer: FC<InjectedDefaultLayoutProps> = ({ developerMode, query, openIn, language, pageMeta, filterByDate, filterByTopics }) => {
	...
	const {API, bookmarks, setBookmarks} = useAPI()
	...
	const bookmarksUUIDs = new Set(bookmarks.map( e => e.uuid))

	const handleBookmarkSwitch = (newVal: boolean) => {
		setViewBookmarksOnly(newVal)
		setCurrentPage(1)
	}
	const handleBookmarkToggle = (row: DataType) => (event: React.MouseEvent) => {
		event.stopPropagation()
		event.preventDefault()
		if (bookmarksUUIDs.has(row.uuid))
			setBookmarks(bookmarks.filter( e => e.uuid !== row.uuid))
		else
			setBookmarks([...bookmarks, row])
	}

	return (
		<>
		<div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', height: 64 }}>
			<Switch onChange={ handleBookmarkSwitch } checkedChildren="Bookmarked" unCheckedChildren="All" style={{ width: 200, margin: 0, marginLeft: filterByTopics ? 24 : 0 }}/>
			{
				filterByTopics && (
					<Select
						mode="multiple"
						style={{ width: '100%', margin: 20, marginRight: 12 }}
			...
			response && response.data.map(
				(row, index) => (
					<a key={row.uuid} href={row['File URL']} onClick={ e => e.preventDefault() }>
						<div
							style={{ height: ROW_HEIGHT, width: '100%', paddingLeft: 4, paddingRight: 12 }}
							onClick={ handlePDFOpen(openIn, row['File URL'])}
						>
							<div style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between', width: '100%' }} >
								<span style={{whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>
									<Button style={{borderWidth: 0}} onClick={ handleBookmarkToggle(row) }> {bookmarksUUIDs.has(row.uuid) ? <StarFilled /> : <StarOutlined />} </Button>
									<h3 style={{ fontSize: 16, display: 'inline'}}> {language === 'english' ? row['Title in English'] : row['Title in Original Language']} </h3>
									{/* <Button style={{borderWidth: 0}} onClick={ handlePDFOpen(openIn, row['File URL'], true)}> <Icon component={NewTabIcon}/> </Button> */}
								</span>
		...
		<Pagination style={{ alignSelf: 'center' }} disabled={!response} current={currentPage} simple pageSize={ITEMS_PER_PAGE} total={ response.count || 1} onChange={handlePageChange}/>
		</>
	)
}
